...
def restart_uwsgi():
    ...
    with settings(warn_only=True):
        ...
        sudo('kill -9 `cat /tmp/project-master_django-project-seed.pid`')
        sudo('rm /tmp/project-master_django-project-seed.pid /tmp/uwsgi_django-project-seed.sock')
...
def dump_db_snapshot():
    ...
    sudo('mysqldump --user REMOTE_DB_USER --password dbdjango-project-seed > %s' % (remote_tmp_file_path))
